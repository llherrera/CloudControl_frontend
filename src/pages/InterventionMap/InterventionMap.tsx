...
const Section = () => {
  ...
  // icon cache persisted across renders
  const iconCacheRef = useRef<Record<string, Icon>>({});
  ...
  // create a colored SVG icon as data URL (cached)
  const createColoredIcon = (color: string) => {
    const cache = iconCacheRef.current;
    if (cache[color]) return cache[color];

    // simple marker SVG: pin with white center
    const svg = `
      <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' width='36' height='36'>
        <path fill='${color}' stroke='#000000' stroke-width='0.6' d='M12 2C8.1 2 5 5.1 5 9c0 5.3 7 13 7 13s7-7.7 7-13c0-3.9-3.1-7-7-7z'/>
  ...
  useEffect(() => {
    ...
    const markersList = locs.map(loc => {
      ...
      // Buscar secretaria por nombre (ajusta si quieres otro criterio)
      ...
      const icon = createColoredIcon(secColor);

      return (
        <Marker
          key={`${lat}-${lng}-${loc.date ?? Math.random()}`}
          position={[lat, lng]}
          icon={icon}
        >
          <Popup>
      ...
      dispatch(thunkGetMapZoomByPlan(id_plan))
        ...
        .then((zoomStr) => {
          ...
          if (!isNaN(zoomParsed)) {
            ...
            console.warn('zoomParsed no es un número válido:', zoomStr);
          }
        })
        .catch((error) => {
          console.error('Error al obtener zoom por defecto:', error);
