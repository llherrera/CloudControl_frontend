...
export const ActivityForm = ({ showPlan }: { showPlan: ActionPlan }) => {
    ...
    const validateActionFields = (): boolean => {
        ...
        if (totalNodes > totalP) newErrors['Total'] = 'Los valores de las metas de producto no pueden superar a los valores de las actividades programadas';
        setErrors(newErrors);
        return Object.keys(newErrors).length === 0;
    };
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!validateActionFields()) {
            ...
            if (activities.length === 0 || activities[0].id_activity === 0) {
...
export const UpdateActivityForm = () => {
    ...
    const { plan, levels, loadingActivityActionPlan, selectedPlan, secretaries } = useAppSelector(store => store.plan);

    if (!selectedPlan) return <p>Cargando plan</p>

    const [actionPlan, setActionPlan] = useState<ActionPlan>(selectedPlan);
    const [listNodes, setListNodes] = useState<UnitNodeInterface[]>([]);
    const [selectNodes, setSelectNodes] = useState<UnitNodeInterface[]>([]);
    const [selectNodesFinan, setSelectNodesFinan] = useState<number[]>([]);
    const [errors, setErrors] = useState<{ [key: string]: string }>({});
